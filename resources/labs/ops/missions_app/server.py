...
# X-Context: Database course - Operations lab - Source code for "missions" web application
...
for dbms_name in ['postgresql', 'opensearch']:
    dbms_settings = raw_configuration[dbms_name + '_settings']

    if not isinstance(dbms_settings, dict):
        raise Exception(f'Option "{dbms_name}_settings" must be parseable as a dictionary')
    
    for required_key in ['remote_hosts', 'user', 'password']:
        if not required_key in dbms_settings.keys():
            raise Exception(
                f'Missing {dbms_name} option/key "{required_key}": ' + repr(raw_configuration))

    if not isinstance(dbms_settings['remote_hosts'], list):
        raise Exception(
            f'"remote_hosts" option for {dbms_name} must be a list: ' + repr(raw_configuration))
...
postgresql_hosts = ','.join(postgresql_settings['remote_hosts'])
...
opensearch_hosts = []
for remote_host in opensearch_settings['remote_hosts']:
    opensearch_hosts.append({'host': remote_host, 'port': 9200})

try:
    s = opensearch_settings
    opensearch_client = opensearch(
        hosts=opensearch_hosts,
        http_auth=(opensearch_settings['user'], opensearch_settings['password']),
        use_ssl=True, verify_certs=False)

    opensearch_server_information = opensearch_client.info()
    _log.debug('Connected to OpenSearch cluster: ' + repr(opensearch_server_information))
...
def handle_report_submission():
    ...
    # ---------------------------------------------------------------------------------------------
    try:
        request_data['submission_datetime'] = time.ctime()
        ...
        with postgresql_client.cursor() as cursor:
            ...
            cursor.execute(
                ...
                ') VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s) RETURNING id', (
                    s['submission_datetime'], s['start_datetime'], s['end_datetime'],
                    s['code_name'], s['budget'], s['cost'], s['location']['lat'],
                    s['location']['lon'], app_user, s['report']))
            ...
            _log.debug('Commiting database changes to PostgreSQL')
            postgresql_client.commit()

        # -----------------------------------------------------------------------------------------
        _log.info('Storing received report in OpenSearch')
        opensearch_client.index(index='missions', body=request_data)
